      ...
      // test randomness
      {
          ...
          for (int i = 0; i < RUN_TESTS; i++) {
            ...
            for (int j = 0; j < RANDOM_REPEATS; j++)
            {
                ...
                BOOST_CHECK(testWallet.SelectCoinsMinConf(0.5 * COIN, filter_standard, GroupCoins(vCoins), setCoinsRet , nValueRet, coin_selection_params, bnb_used));
                BOOST_CHECK(testWallet.SelectCoinsMinConf(0.5 * COIN, filter_standard, GroupCoins(vCoins), setCoinsRet2, nValueRet, coin_selection_params, bnb_used));
